...
function formatTime(minutes, seconds) {
  ...
  const str = '';
  return str.concat(min.padStart(2, '0'), ':', sec.padStart(2, '0'));
}
...
function isPalindrome(str) {
  const letter1 = str.toLowerCase().replace(/[\s.,!?%]/g, '');
  const letter2 = letter1.split('').reverse().join('');
  return letter1.includes(letter2);
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    if (maxLength === words[i].length) res = words[i];
    if (res) break;
  }
  return res;
}
